...
module Amp
  module Mercurial
    module Repositories
      ...
      class Repository < Amp::Core::Repositories::AbstractLocalRepository
      ...
      class MercurialPicker < Amp::Core::Repositories::GenericRepoPicker
        def pick(config, path='', create=false)
          # hot path so we don't load the HTTP repos!
          unless path[0,4] == "http"
            return Repositories::LocalRepository.new(find_repo(path), create, config)
          end
          return Repositories::HTTPSRepository.new(path, create, config) if path[0,5] == "https"
          return Repositories::HTTPRepository.new(path, create, config)  if path[0,4] == "http"
        end
